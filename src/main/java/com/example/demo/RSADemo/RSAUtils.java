package com.example.demo.RSADemo;
...
public class RSAUtils {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ByteBuf p = Unpooled.buffer(4096);
        ByteBuf pr = Unpooled.buffer(4096);
        p.writeBytes(publicKey);
        pr.writeBytes(privateKey);
        System.out.println("公钥："+ Base64.encode(p));
        System.out.println("私钥："+ Base64.encode(pr));

        byte[] encryptByPublicKey = encryptByPublicKey(password.getBytes(), publicKey);
        ByteBuf eb = Unpooled.copiedBuffer(encryptByPublicKey);
        System.out.println("使用公钥加密后的数据："+Base64.encode(eb));
